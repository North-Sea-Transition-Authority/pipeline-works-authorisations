...
-- order is important so tables restored in an foreign key friendly order
  l_backup_tables bpmmgr.varchar2_list_type := bpmmgr.varchar2_list_type(
    ...
    l_delete_from_table_statements bpmmgr.varchar2_list_type := bpmmgr.varchar2_list_type();
  ...
  FOR backup_table IN (
    ...
    FROM TABLE(l_backup_tables)
      ...
      BEGIN
        ...
        l_delete_from_table_statements.EXTEND;
        l_delete_from_table_statements(l_delete_from_table_statements.LAST) := 'DELETE FROM ' || l_backup_table_dest_ref || ' WHERE ' || l_pk_column || ' NOT IN (SELECT ' || l_pk_column || ' FROM ' || l_backup_table_src_ref ||  ');';
        ...
        FOR backup_column IN(
          ...
          WHERE column_value != l_pk_column 
        ) LOOP
        ...
        l_backup_table_src_merge_text := l_backup_table_src_merge_text || ' WHEN NOT MATCHED THEN INSERT (' ||
          st.join(p_strings=>l_restore_column_names, p_delimiter=> ',') || ')' || CHR(10) || 
          'VALUES (src.' || st.join(p_strings=>l_restore_column_names, p_delimiter => ', src.') || ')';
    ...
    -- remove rows in tables not found in the backed up set.
    -- do deletes in reverse order of restore to avoid fk constraint issues.
    FOR idx IN 1.. l_delete_from_table_statements.COUNT
    LOOP

      dbms_output.put_line(l_delete_from_table_statements((l_delete_from_table_statements.COUNT - (idx-1))));
...
-- manual sequence updates where identity columns not used
DECLARE
  l_backup_schema VARCHAR2(400) := :p_backup_schema_uppercase;
  
   l_max_section_clause_id NUMBEr;
   l_max_section_clause_vers_id NUMBER;
   
   l_next_val NUMBER;
   l_cur_val NUMBER;
  BEGIN
      
  EXECUTE IMMEDIATE ('SELECT MAX(id) FROM ' || l_backup_schema || '.DT_SECTION_CLAUSES')
  INTO l_max_section_clause_id;
      
  EXECUTE IMMEDIATE ('SELECT MAX(id) FROM ' || l_backup_schema || '.DT_SECTION_CLAUSE_VERSIONS')
  INTO l_max_section_clause_vers_id;
      
      
  EXECUTE IMMEDIATE 'ALTER SEQUENCE ' || l_backup_schema || '.dt_section_clauses_id_seq INCREMENT BY ' || l_max_section_clause_id;
     
  EXECUTE IMMEDIATE 'ALTER SEQUENCE ' || l_backup_schema || '.dt_scv_id_seq INCREMENT BY ' || l_max_section_clause_vers_id;

  -- select next val so the sequences update
  EXECUTE IMMEDIATE ('SELECT ' || l_backup_schema || '.dt_section_clauses_id_seq.NEXTVAL FROM dual')
  INTO l_next_val;

  -- select next val so the sequences update
  EXECUTE IMMEDIATE ('SELECT ' || l_backup_schema || '.dt_scv_id_seq.NEXTVAL FROM dual')
  INTO l_next_val;

  -- reset increment value to restore expected behaviour.
  EXECUTE IMMEDIATE 'ALTER SEQUENCE ' || l_backup_schema || '.dt_section_clauses_id_seq INCREMENT BY 1';
  
  EXECUTE IMMEDIATE 'ALTER SEQUENCE ' || l_backup_schema || '.dt_scv_id_seq INCREMENT BY 1';

INSERT INTO ${datasource.user}.DOCUMENT_TEMPLATES (Mnem) VALUES ('CCUS_CONSENT_DOCUMENT');
INSERT INTO
    ${datasource.user}.DT_SECTIONS (DT_ID, NAME, STATUS, START_TIMESTAMP)
    (SELECT
         (SELECT ID FROM ${datasource.user}.DOCUMENT_TEMPLATES WHERE Mnem = 'CCUS_CONSENT_DOCUMENT') ,
         NAME,
         STATUS,
         CURRENT_TIMESTAMP
     FROM
...
CREATE PRIVATE TEMPORARY TABLE ora$ptt_ccus_petroleum_mapping (
    ccus_section_clause_id NUMBER,
...
BEGIN
    FOR record IN (SELECT dtsc.ID, dtsc.S_ID, dts.NAME, dt.mnem
        ...
        LOOP
            ...
            INSERT INTO ora$ptt_ccus_petroleum_mapping VALUES (new_id, record.ID);
            ...
            WHERE
                    ...
                    AND dt2.mnem = 'CCUS_CONSENT_DOCUMENT';
            ...
            SELECT
                ...
                dtscv.NAME,
                dtscv.TEXT,
                (SELECT ccus_section_clause_id FROM ora$ptt_ccus_petroleum_mapping WHERE petroleum_section_clause_id = dtscv.PARENT_SC_ID),
                LEVEL_ORDER,
                STATUS,
                CURRENT_TIMESTAMP,
                1

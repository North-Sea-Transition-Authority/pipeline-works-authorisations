    ...
    <@fdsForm.htmlForm>
      ...
      <h2 class="govuk-heading-l">Environmental</h2>
        ...
        <#if availableQuestions?seq_contains("BEIS_EMT_PERMITS")>
          ...
          <@fdsFieldset.fieldset legendHeading="DESNZ EMT" legendHeadingSize="h3" legendHeadingClass="govuk-fieldset__legend--m">

              <@fdsRadio.radioGroup path="form.emtHasSubmittedPermits" labelText="Have you submitted any relevant environmental permits to DESNZ EMT?" fieldsetHeadingSize="h4" hiddenContent=true>
                  <@fdsRadio.radioYes path="form.emtHasSubmittedPermits">
                      <@fdsTextarea.textarea path="form.permitsSubmitted" nestingPath="form.permitsSubmitted" labelText="Which permits have you submitted to DESNZ?" hintText="Include the date submitted for each permit" characterCount=true maxCharacterLength=maxCharacterLength?c/>
                  </@fdsRadio.radioYes>
                  <@fdsRadio.radioNo path="form.emtHasSubmittedPermits"/>
              </@fdsRadio.radioGroup>

              <@fdsRadio.radioGroup path="form.emtHasOutstandingPermits" labelText="Do you have any environmental permits that have not yet been submitted to DESNZ EMT?" fieldsetHeadingSize="h4" hiddenContent=true>
                  <@fdsRadio.radioYes path="form.emtHasOutstandingPermits">
                      <@fdsTextarea.textarea path="form.permitsPendingSubmission" labelText="Which permits have you not submitted to DESNZ?" nestingPath="form.permitsPendingSubmission" characterCount=true maxCharacterLength=maxCharacterLength?c/>
                      <@fdsDateInput.dateInput formId="emtSubmissionDay" dayPath="form.emtSubmissionDay" monthPath="form.emtSubmissionMonth" yearPath="form.emtSubmissionYear" labelText="What is the latest date all relevant environmental permits will be submitted to DESNZ?" hintText="DESNZ will require these permits prior to their assessment of your PWA" fieldsetHeadingSize="h5" defaultHint=false/>
                  </@fdsRadio.radioYes>

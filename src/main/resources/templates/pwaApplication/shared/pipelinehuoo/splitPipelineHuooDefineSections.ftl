...
<@defaultPage htmlTitle=pageHeading pageHeading=pageHeading breadcrumbs=false fullWidthColumn=true twoThirdsColumn=false>
...
<#macro sectionStartPointOptions listPath pickableIdentOptions sectionNumberIndex isLastSection>
  ...
  <@fdsRadio.radioGroup
    labelText="Where does the section start?"
    path=identLocationOptionPath
    fieldsetHeadingClass="govuk-fieldset__legend--s"
    fieldsetHeadingSize="h3">
      <#local isFirstIdentOption=true/>
      <#list pickableIdentOptions as optionPickableString, optionDisplayableString >
          <@fdsRadio.radioItem path=identLocationOptionPath itemMap={optionPickableString: optionDisplayableString} isFirstItem=isFirstIdentOption/>
          <#local isFirstIdentOption=false/>
      </#list>
  </@fdsRadio.radioGroup>

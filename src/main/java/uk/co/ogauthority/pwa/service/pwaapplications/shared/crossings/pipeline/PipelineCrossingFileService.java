...
public class PipelineCrossingFileService implements ApplicationFormSectionService {
  ...
  /**
   * Create and persist a new pipeline crossing file linked to app detail and uploaded file id.
   */
  private PadPipelineCrossingFile createAndSavePipelineCrossingFile(PwaApplicationDetail pwaApplicationDetail,
                                                              String uploadedFileId) {
    var newFileLink = new PadPipelineCrossingFile(
    ...
    return padPipelineCrossingFileRepository.save(newFileLink);
  }

  /**
   * Return linked pipeline crossing if it exists for application detail else throw not found exception.
   */
  public PadPipelineCrossingFile getPipelineCrossingFile(String fileId, PwaApplicationDetail pwaApplicationDetail) {
    return padPipelineCrossingFileRepository.findByPwaApplicationDetailAndFileId(
        pwaApplicationDetail,
        fileId
    )
  ...
  /**
   * Remove pipeline crossing file link and delete uploaded file.
   */
  @Transactional
  void deletePipelineCrossingFilesAndLinkedUploads(Iterable<PadPipelineCrossingFile> filesToBeRemoved,
                                                WebUserAccount user) {
    for (PadPipelineCrossingFile fileToRemove : filesToBeRemoved) {
    ...
    padPipelineCrossingFileRepository.deleteAll(filesToBeRemoved);
  }
   ...
   */
  @Transactional
  void deletePipelineCrossingFileLink(PadPipelineCrossingFile fileToRemove) {
    padPipelineCrossingFileRepository.delete(fileToRemove);
  }
  ...
  public void updateOrDeleteLinkedFilesUsingForm(PwaApplicationDetail pwaApplicationDetail,
    ...
    var existingLinkedFiles = padPipelineCrossingFileRepository.findAllByPwaApplicationDetail(pwaApplicationDetail);
    var filesToBeRemoved = new HashSet<PadPipelineCrossingFile>();
    var filesToUpdate = new HashSet<PadPipelineCrossingFile>();
    ...
    padPipelineCrossingFileRepository.saveAll(filesToUpdate);
    deletePipelineCrossingFilesAndLinkedUploads(filesToBeRemoved, user);
   ...
   */
  public List<UploadedFileView> getUpdatedPipelineCrossingFileViewsWhenFileOnForm(
      PwaApplicationDetail pwaApplicationDetail,
      CrossingDocumentsForm form) {

    return updateFormWithSuppliedUploadedFileViews(
        form,
        () -> getPipelineCrossingFileViews(pwaApplicationDetail, ApplicationFileLinkStatus.ALL));
  ...
  public void deleteUploadedFileLink(String fileId, PwaApplicationDetail pwaApplicationDetail) {
    PadPipelineCrossingFile existingFile = getPipelineCrossingFile(fileId,
        pwaApplicationDetail);
    deletePipelineCrossingFileLink(existingFile);
  }

  /**
   * Method which creates "temporary" link to application detail pipeline crossing file
  ...
  public void createUploadedFileLink(String uploadedFileId, PwaApplicationDetail pwaApplicationDetail) {
    createAndSavePipelineCrossingFile(
        pwaApplicationDetail,
        uploadedFileId
    );
  }

...
public class HuooVariationTaskListController {
  ...
  @Autowired
  public HuooVariationTaskListController(
  ...
  public ModelAndView viewTaskList(@PathVariable("applicationId") Integer applicationId, AuthenticatedUserAccount user) {
    ...
    if (application.getApplicationType() != PwaApplicationType.HUOO_VARIATION) {
      throw new PwaEntityNotFoundException("Application of wrong type:" + application.getApplicationType());
    }

    var modelAndView = new ModelAndView("pwaApplication/huooVariation/huooTaskList")

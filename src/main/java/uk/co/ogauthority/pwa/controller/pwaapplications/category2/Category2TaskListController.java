...
public class Category2TaskListController {
  ...
  @Autowired
  public Category2TaskListController(
  ...
  private List<TaskListEntry> getApplicationTaskList(PwaApplication pwaApplication) {
    return new ArrayList<>(List.of(
        new TaskListEntry("No tasks", pwaApplicationRedirectService.getTaskListRoute(pwaApplication, null), false)
    ));
  }
  ...
  public ModelAndView viewTaskList(@PathVariable("applicationId") Integer applicationId, AuthenticatedUserAccount user) {
    ...
    if (application.getApplicationType() != PwaApplicationType.CAT_2_VARIATION) {
      throw new PwaEntityNotFoundException("Application of wrong type:" + application.getApplicationType());
    }

    var modelAndView = new ModelAndView("pwaApplication/category2/cat2TaskList")
